...
hashed_email: str = Security.encrypt_email("sBcEY@example.com")
...
def test_text_to_sql_returns_sql(mock_create) -> None:
    ...
    sql = text_to_sql(question, db_schema, hashed_email, key_to_use)
...
def test_text_to_sql_handles_empty_response(mock_create) -> None:
    ...
    sql = text_to_sql(
        "Anything", [{"table": "x", "columns": ["y"]}], hashed_email, key_to_use)
...
def test_text_to_sql_handles_empty_schema(mock_create) -> None:
    ...
    sql = text_to_sql("Something", [], hashed_email, key_to_use)
    assert "SELECT" in sql.upper()
...
def test_text_to_sql_handles_api_failure(mock_create) -> None:
    ...
    sql = text_to_sql(
        "Fail case", [{"table": "x", "columns": ["y"]}], hashed_email, key_to_use)
